...
class StartHandler(BaseHandler):
    ...
    async def __call__(self, message: types.Message) -> None:
        ...
        await self.log_message(chat_id, message.message_id, -1, self, 'start_command')
...
class HelpHandler(BaseHandler):
    ...
    async def __call__(self, message: types.Message) -> None:
        chat_id=message.from_user.id
        await self.log_message(chat_id, message.message_id, -1, self, 'help_command')
...
class MemberStatus(BaseHandler):
    ...
    async def __call__(self, update: types.ChatMemberUpdated) -> None:
        ...
        await self._db.update_activity(chat_id, activity_status)
        await self.log_info(chat_id, -1, -1, self, f'{activity_status}')
...
class UpdateContact(BaseHandler):
    ...
    async def __call__(self, message: types.Contact) -> None:
        ...
        await self._db.update_phone_number(chat_id, phone_number)
        await self.log_message(chat_id, message.message_id, -1, self, f'phone_number: {phone_number}')
