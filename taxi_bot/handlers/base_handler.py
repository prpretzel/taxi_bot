...
class BaseHandler:
    ...
    async def create_user(self, message):
        phone_number = await self._db.create_user(message)
    ...
    async def show_active_orders(self, driver_id):
        active_orders = await self._db.get_orders(100)
    ...
    async def delete_old_messages(self, order_id=None, chat_id=None, message_id=None):
        orders = await self._db.get_order_messages(order_id=order_id, chat_id=chat_id, message_id=message_id)
        for order in orders:
            chat_id, message_id, order_id = order.chat_id, order.message_id, order.order_id
            try:
                await self._bot.delete_message(chat_id, message_id)
            except:
                await self.log_error(chat_id, message_id, order_id, self, f'can`t delete chat_id: {chat_id} message_id: {message_id}')
        await self._db.update_log_status(log_ids=[order.log_id for order in orders])
    ...
    async def send_message(self, chat_id, order_id, text, kb_name=None):
        ...
        await self.log_message(chat_id, message.message_id, order_id, self, f'text: {text}')
        return message
    ...
    async def edit_reply_markup(self, callback_query: types.CallbackQuery, kb_name, order_id):
        ...
        await self.log_info(chat_id, message.message_id, order_id, self, f'edit_kb: {kb_name}')
    ...
    async def remove_reply_markup(self, query, order_id):
        ...
        await self.log_info(chat_id, message.message_id, order_id, self, 'remove_kb')
    ...
    async def send_venue(self, chat_id, lat, lon, destination, price, order_id, kb_name=None):
        ...
        await self.log_message(chat_id, message.message_id, order_id, self, f'lat: {lat}; lon: {lon} dest: {destination}; price: {price}')
    
    async def log_info(self, chat_id, message_id, order_id, _self, message):
        await self._db.log_message('INFO', chat_id, message_id, order_id, _self, message, 0)
    
    async def log_message(self, chat_id, message_id, order_id, _self, message):
        await self._db.log_message('MESSAGE', chat_id, message_id, order_id, _self, message, 1)
    
    async def log_error(self, chat_id, message_id, order_id, _self, message):
        await self._db.log_message('ERROR', chat_id, message_id, order_id, _self, message, 0)
